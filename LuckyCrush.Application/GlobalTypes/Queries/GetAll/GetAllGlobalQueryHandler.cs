...
public class GetAllGlobalQueryHandler(ILogger<GetAllGlobalQueryHandler> logger, IMapper mapper,
    IGlobalTypeRepository globalTypeRepository) : IRequestHandler<GetAllGlobalQuery, IEnumerable<GlobalTypeDto>>
{
    public async Task<IEnumerable<GlobalTypeDto>> Handle(GetAllGlobalQuery request, CancellationToken cancellationToken)
    {
