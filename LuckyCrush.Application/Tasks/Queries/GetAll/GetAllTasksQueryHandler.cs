...
public class GetAllTasksQueryHandler(ILogger<GetAllTasksQueryHandler> logger, IMapper mapper,
    ITaskRepository taskRepository) : IRequestHandler<GetAllTasksQuery, Result<IEnumerable<GoalTaskDto>>>
{
    public async Task<Result<IEnumerable<GoalTaskDto>>> Handle(GetAllTasksQuery request, CancellationToken cancellationToken)
    {
        logger.LogInformation("Getting all tasks");
        var tasks = await taskRepository.GetAllAsync();

        var results = mapper.Map<IEnumerable<GoalTaskDto>>(tasks);
        return Result<IEnumerable<GoalTaskDto>>.Success(results);
    }
}
